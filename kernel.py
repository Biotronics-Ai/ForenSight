#kernel.py
# ===========================================================
# Kernel utilities for DNASample vectors
# ===========================================================
...
class KernelMatrix:
    """
    Builds a full kernel (inner product) matrix over DNASample vectors and
    provides distance-based retrieval. Each DNASample.sequence can be 1-D
    ...
    Args:
        samples: Ordered list of DNASample objects to include in the kernel.
    ...
    def __init__(
        ...
        *,
        ...
        if not samples:
            raise ValueError("KernelMatrix requires at least one DNASample.")
        if not memmap_dir:
    ...
    def build_from_batches(
        cls,
        batches: Iterable[List[DNASample]],
        *,
        matrix_memmap_path: Optional[str] = None,
        seq_memmap_path: Optional[str] = None,
        memmap_dir: str = "",
        ...
        """
        Build a kernel from batches of DNASample objects while preserving order.
    ...
    def _build_kernel(
        ...
        *,
        ...
        Args:
            samples: DNASamples (order preserved).
        ...
        logger.info("Stack prepared with %d vectors of length %d and feature_dim %d", len(vectors), base_length, feature_dim)
    ...
    def best_match(self, target_id: str) -> Optional[Tuple[str, float]]:
        """
        Return the closest sample to target_id (excluding itself) based on
        Euclidean distance derived from the kernel matrix.

        Args:
            target_id: id to match against.

        Returns:
            (id, distance) of the closest other sample, or None if only one sample.
        """
